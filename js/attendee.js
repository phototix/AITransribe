        ...
        async function loadHistory() {
            ...
            if (response.success && response.history.length > 0) {
                ...
                response.history.forEach(item => {
                    ...
                    historyItem.innerHTML = `<small style="display:none;padding:2px;margin:2px;" class="text-muted alert alert-warning">${new Date(item.timestamp).toLocaleString()}</small><div class="translation-text" style="margin-top:5px;margin-bottom:15px;">${item.translated_text}</div>`;
